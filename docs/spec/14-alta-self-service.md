...
- **Visibilidad de campos (2026-09-23, corrección UX):** de estos, `rfc`, `razon_social`, `regimen_fiscal`, `codigo_postal` y `pais` son obligatorios y siempre visibles (`pais` es un selector fijo con un único valor, "México" → `MEX`, no texto libre — ver `08-decisions.md`). `calle`, `no_exterior`, `no_interior`, `colonia`, `municipio` y `estado` son opcionales tanto en el schema cliente (`registroSchemas.ts`) como en el backend (columnas nullable de `ventas_registros_self_service`) y van agrupados bajo un toggle colapsable **"Domicilio fiscal (opcional)"**, cerrado por defecto, para no mostrar 11 campos cuando solo 5 son obligatorios. El toggle arranca abierto si el usuario ya había capturado algo en esos campos (por ejemplo, al regresar al paso 1) — no debe esconderle datos que ya escribió. Es un `useState` local en `DatosFiscalesStep.tsx`, sin agregar dependencias (no hay `Collapsible` de Radix instalado, y `CLAUDE.md` no permite librerías fuera de la lista aprobada).
